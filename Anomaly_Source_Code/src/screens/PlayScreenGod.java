...
import testeGUi.StuffFactory;
import testeGUi.FieldOfView;
import testeGUi.Tile;
import testeGUi.World;
import testeGUi.WorldBuilder;

public class PlayScreenGod implements Screen {
	...
	private void createCreatures(StuffFactory factory){
		player = factory.newGodPlayer(messages, fov);
		...
		for (int z = 0; z < world.depth(); z++){
			...
			for (int i = 0; i < z * 2 + 1; i++){
				...
				factory.newVampire(z,player);
				factory.newGoblin(z, player);
			}
			for (int i = 0; i < z * 1 + 1; i++) {
				factory.newSkinCrawler(z, player);
			}
		}
	}
	...
	public Screen respondToUserInput(KeyEvent key) {
		...
		if (player.hp() < 1) {
			death++;
			return new LoseScreenGod(player);
		}
